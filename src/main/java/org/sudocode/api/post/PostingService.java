...
public class PostingService {
    ...
    public Page<ProjectView> fetchAllProjectViews(@Nullable String title,
                                                  @Nullable String difficultyStr, @Nullable String description,
                                                  Pageable pageable) {

        Difficulty difficultyEnum = getDifficulty(difficultyStr);

        return projectRepo.filterAll(title, difficultyEnum, description, pageable);
    }

    public Difficulty getDifficulty(@Nullable String difficultyStr) {
        return isNotBlank(difficultyStr)
                ? difficultyEnumFromValue(difficultyStr) : null;
    }
    ...
    public Project updateProject(Long id, Project newProject, User currentUser) {
        return projectRepo.fetchById(id).filter(p -> p.isPostedBy(currentUser))
                          .map(project -> {
    ...
    public Comment updateComment(Comment newComment, Long commentId, Long projectId,
                                 User currentUser) {
        return commentRepo.fetchById(commentId).filter(c -> c.isPostedBy(currentUser))
                          .map(comment -> {
                    ...
                    newComment
                            .setId(commentRepo.existsById(commentId) ? null : commentId);
     ...
     * {@link org.sudocode.api.core.security.timeout.TimeOutService}.
     *
     * <p>
    ...
    public LocalDateTime fetchLatestPostDateByAuthorId(Long id) {
        var lastCommentDate = commentRepo.fetchLatestPostDateByAuthorId(id)
                                         .orElse(DEFAULT_LOCAL_DATE_TIME);
        var lastPostDate = projectRepo.fetchLatestPostDateByAuthorId(id)
                                      .orElse(DEFAULT_LOCAL_DATE_TIME);

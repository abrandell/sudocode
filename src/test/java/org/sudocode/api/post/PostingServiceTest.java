...
class PostingServiceTest {
	...
	void setUp() {
		...
		commentViewMock1 = CommentViewMock.builder().id(1L).author(userViewMock1)
				.body(comment1.getBody()).build();
	...
	void updatedProject_sameAuthor_thenUpdate() {
		...
		Project updated = Project.builder(user1).id(project1.getId())
				.title("updated-title").difficulty(EXPERT).build();
	...
	void fetchAllProjectViews() {
		...
		assertEquals(postingService.fetchAllProjectViews(null, null, null, null)
	...
	void fetchProjectViewById() {
		given(projectRepo.findViewById(project1.getId()))
				.willReturn(Optional.of(projectViewMock1));
		...
		assertAll("Fetch view by id",
				() -> assertEquals(found.getDifficulty(),
						projectViewMock1.getDifficulty(), "Difficulties should match"),
				() -> assertThrows(ProjectNotFoundException.class,
						() -> postingService.fetchProjectViewById(-1L),
	...
	void updateProject_sameAuthor_thenUpdate() {
		...
		final Project original = Project.builder(project1.getAuthor())
				.id(project1.getId()).description(project1.getDescription())
				.difficulty(project1.getDifficulty()).title(project1.getTitle()).build();
		...
		final Project result = postingService.updateProject(
				originalId, Project.builder(user1).title("new title")
						.description("new description").difficulty(EXPERT).build(),
				this.user1);

		assertAll("Project update -- same author",
				() -> assertEquals(original.getId(), result.getId(),
						"ID's should match."),
				() -> assertEquals(user1, result.getAuthor(), "Users should match"),
	...
	void postComment_givenUnknownProjectId_ThenException() {
		...
		assertAll("Posting comment with project ID not found", () -> assertThrows(
	...
	void updateComment_notSameAuthor_thenPostNew() {
		...
		var result = postingService.updateComment(newComment, comment1.getId(),
				project1.getId(), user2);
	...
	void updateComment_sameAuthor_thenUpdate() {
		...
		Comment updated = postingService.updateComment(newComment, originalId,
				project1.getId(), user1);
	...
	void fetchLatestPostDateByAuthorId_noLastPost() {
		given(projectRepo.fetchLatestPostDateByAuthorId(any()))
				.willReturn(Optional.empty());
		given(commentRepo.fetchLatestPostDateByAuthorId(any()))
				.willReturn(Optional.empty());
	...
	void fetchLatestDate_lastPostAvailable() {
		...
		given(commentRepo.fetchLatestPostDateByAuthorId(1L))
	...
	void testFilterAll_invalidDifficulty_thenException() {
		assertThrows(InvalidDifficultyException.class, () -> postingService
				.fetchAllProjectViews(null, "invalid-diff", null, null));
	}
